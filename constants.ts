...
export const SECTION_CONTENT = {
    hero: {
        ...
        brand: {
            crest: 'Clear Seas Crest',
            blueprint: 'Inline blueprint grid with parallax micro-cards',
            cta: {
                label: 'View the Clear Seas Blueprint',
                href: '#clear-seas',
                note: 'Brand-first hero focuses on the crest + blueprint pairing while keeping the visualizer unobstructed.',
            },
            microCards: [
                { title: 'Crest', detail: 'Inline crest badge with cyan lattice + logo bursts.', token: 'crest' },
                { title: 'Blueprint', detail: 'Mini grid of parallax cards that react to pointer tilt.', token: 'grid' },
                { title: 'CTA rail', detail: 'Aligned CTAs keep glass minimal so the backdrop breathes.', token: 'cta' },
            ],
        },
        focusContent: {
...
export const LAB_HANDSHAKE: Record<SectionId, { phase: string; palette: string; caption: string; cue: string; mask: number; blur: number; }[]> = {
    hero: [
        { phase: 'Phase 01 · Morph', palette: 'linear-gradient(135deg, rgba(103,232,249,0.28), rgba(76,29,149,0.2))', caption: 'Morph canvas leans into cyan crest beams with shallow blur.', cue: 'crest hue', mask: 3, blur: 1 },
        { phase: 'Phase 02 · Crystal', palette: 'linear-gradient(135deg, rgba(14,165,233,0.25), rgba(236,72,153,0.22))', caption: 'Crystal canvas sharpens edges with labeled palette chips.', cue: 'crystal', mask: 4, blur: 2 },
        { phase: 'Phase 03 · Invert', palette: 'linear-gradient(135deg, rgba(59,130,246,0.2), rgba(16,185,129,0.22))', caption: 'Dual-visualizer handshake syncs mask intensity for clean invert.', cue: 'invert', mask: 2, blur: 1 },
    ],
    philosophy: [],
    pillars: [],
    quality: [],
...
export interface EmergencyJourneySession {
    label: string;
    title: string;
    detail: string;
    cues: string[];
...
export interface EmergencyJourneyMetric {
    label: string;
    value: string;
    note: string;
}

export interface EmergencyJourney {
    kicker: string;
    title: string;
    summary: string;
    status: string;
    sessions: EmergencyJourneySession[];
    metrics: EmergencyJourneyMetric[];
    cta: { label: string; href: string; };
    reset: { patternIntensity: number; rotationSpeed: number; glitchIntensity: number; gridDensity: number; };
}

export const EMERGENCY_JOURNEY: EmergencyJourney = {
    kicker: 'Emergency TA Tile',
    title: 'Triage & Orchestrate // Three-Session Rally',
    summary: 'An always-on emergency tile that choreographs the VIB34D visualizer, dispatches comms, and stages recovery content in three decisive sessions.',
    status: 'READY · LIVE',
    reset: { patternIntensity: 1.15, rotationSpeed: 0.32, glitchIntensity: 0.028, gridDensity: 26 },
    sessions: [
        {
            label: 'Session 01',
            title: 'Stabilize & Signal',
            detail: 'Lock the viewport, pulse a cyan beacon, and route users to the emergency focus-content without losing navigation context.',
            cues: ['fixed viewport', 'beacon beam', 'scroll guard'],
            gradient: 'linear-gradient(135deg, rgba(103,232,249,0.25), rgba(59,130,246,0.15))',
            intensity: 1.35,
            rotationSpeed: 0.22,
            glitch: 0.04,
            grid: 32,
        ...
        {
            label: 'Session 02',
            title: 'Choreograph Events',
            detail: 'Thread QA, incident notes, and live ops controls inside the tile so readers see the recovery story unfold in real time.',
            cues: ['live ops', 'qa ribbon', 'timeline sync'],
            gradient: 'linear-gradient(135deg, rgba(244,114,182,0.22), rgba(14,165,233,0.2))',
            intensity: 1.55,
            rotationSpeed: 0.42,
            glitch: 0.065,
            grid: 36,
        ...
        {
            label: 'Session 03',
            title: 'Recovery & Debrief',
            detail: 'Hand off to a calmer visual state with orange embers, share the mitigation steps, and open comments for follow-up.',
            cues: ['comments on', 'debrief log', 'cooldown'],
            gradient: 'linear-gradient(135deg, rgba(251,146,60,0.2), rgba(79,70,229,0.15))',
            intensity: 1.25,
            rotationSpeed: 0.28,
            glitch: 0.03,
            grid: 30,
    ...
    metrics: [
        { label: 'Response time', value: '<90s', note: 'From alert to beaconed tile with live copy.' },
        { label: 'Visibility', value: 'Global', note: 'Anchored to hero + revolver nav for instant access.' },
        { label: 'Sync', value: 'Visualizer + Ops', note: 'Safe ramps keep GPU + copy aligned.' },
    ],
    cta: {
        label: 'Open Emergency Journey',
        href: '#emergency-journey',
    },
};
