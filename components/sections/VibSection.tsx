...
import { CONTROL_SYSTEMS, EMERGENCY_JOURNEY, LAB_HANDSHAKE, MASTER_PLAN, PUBLICATION_PULSE, SECTION_CONTENT, SECTION_GESTURES, SECTION_POLISH, SECTION_QA, SECTION_VALIDATION, SectionId, SECTIONS, VISUALIZER_PROFILES } from '../../constants';
...
import FocusContentCard from '../ui/FocusContentCard';
import EmergencyJourneyTile from '../ui/EmergencyJourney';
import HeroBrandPanel from '../ui/HeroBrandPanel';
import LabHandshakePanel from '../ui/LabHandshakePanel';
...
const VibSection: React.FC<VibSectionProps> = ({ sectionId, visualizerRef, isPortrait = false }) => {
    ...
    const polishPasses = useMemo(() => SECTION_POLISH[sectionId] || [], [sectionId]);
    const validation = useMemo(() => SECTION_VALIDATION[sectionId] || [], [sectionId]);
    const publication = useMemo(() => PUBLICATION_PULSE[sectionId], [sectionId]);
    const focusContent = useMemo(() => data.focusContent, [data]);
    const labPhases = useMemo(() => LAB_HANDSHAKE[sectionId] || [], [sectionId]);
    const chipClass = 'px-3 py-1 rounded-full border border-white/10 bg-white/5 text-[11px] font-mono uppercase tracking-[0.25em] text-white/80';
    ...
    return (
        <div className="w-full min-h-screen overflow-visible pb-24 custom-scrollbar relative">
            ...
            <div className="relative z-10 min-h-full px-4 sm:px-6 md:px-10 py-12 space-y-10">
                {/* header */}
                <header className={`grid gap-6 items-start ${isPortrait ? 'grid-cols-1' : 'md:grid-cols-[1.25fr_1fr]'}`}>
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-lg md:text-xl text-slate-200/85 max-w-3xl leading-relaxed">
                                {data.subline}
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.25em] text-slate-300 font-mono">
                            <span className={chipClass}>VIB34D architecture</span>
                            <span className={chipClass}>Revolver navigation</span>
                            <span className={chipClass}>Morphing feature card</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-3 text-xs font-mono uppercase tracking-[0.3em] text-slate-400">
                            <span className="px-3 py-1 rounded-full bg-white/5 border border-white/15">Parallax-ready hero</span>
                            <span className="px-3 py-1 rounded-full bg-white/5 border border-white/15">CTA rail aligned</span>
                        </div>
                    </div>
                    {sectionId === 'hero' ? (
                        <HeroBrandPanel accent={accent} brand={data.brand} />
                    ) : (
                        ...
                        <div className="p-4 rounded-3xl bg-slate-950/60 border border-white/10 backdrop-blur-xl shadow-[0_10px_50px_rgba(0,0,0,0.25)]">
                            ...
                            <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2">
                                {streamCards.map((item) => (
                                    <article
                                        key={item.title}
                                        className="min-w-[260px] snap-start p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-white/30 transition shadow-inner backdrop-blur relative overflow-hidden"
                                    >
                                        ...
                                        <div className="flex flex-wrap gap-2 text-[10px] font-mono uppercase tracking-[0.25em] text-slate-400">
                                            <span className={chipClass}>Service</span>
                                            <span className={chipClass}>Impact</span>
                                        </div>
                                        </div>
                                    </article>
                                ))}
                ...
                {focusContent && (
                    <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr] items-stretch">
                        <FocusContentCard
                            ...
                            onExpandVisual={(expanded) => {
                                visualizerRef.current?.updateParams({
                                    patternIntensity: expanded ? 1.8 : (VISUALIZER_PROFILES[sectionId].patternIntensity ?? 1.1),
                                    rotationSpeed: expanded ? 0.55 : (VISUALIZER_PROFILES[sectionId].rotationSpeed ?? 0.3),
                                    glitchIntensity: expanded ? 0.1 : (VISUALIZER_PROFILES[sectionId].glitchIntensity ?? 0.02),
                                    gridDensity: expanded ? (VISUALIZER_PROFILES[sectionId].gridDensity ?? 24) + 6 : VISUALIZER_PROFILES[sectionId].gridDensity,
                                });
                            }}
                        />
                        ...
                        <div className="p-4 md:p-6 rounded-3xl bg-slate-950/70 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)] flex flex-col gap-4">
                            <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>Focus-content brief</span>
                                </div>
                                <span className="text-slate-500">800vh journey</span>
                            </div>

                            <div className="space-y-3 text-sm text-slate-200">
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                    <div className="text-[11px] uppercase tracking-[0.25em] font-mono text-white/70 mb-1">Why Vib3Code.com</div>
                                    <p className="leading-relaxed">We anchor the flagship scroll to explain our AI vibe-coding doctrine and show community members how morphing experiences stay transparent and reversible.</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                    <div className="text-[11px] uppercase tracking-[0.25em] font-mono text-white/70 mb-1">GSAP cues</div>
                                    <p className="leading-relaxed">Fourteen ScrollTrigger hooks govern gradient swaps, label reveals, and QA ribbons so the 800vh track mirrors our architecture beats.</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                    <div className="text-[11px] uppercase tracking-[0.25em] font-mono text-white/70 mb-1">Rollout plan</div>
                                    <p className="leading-relaxed">The focus-content card will ship with smoke + load tests plus a documentation appendix so contributors can craft their own longform decks.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {labPhases.length > 0 && (
                    <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr] items-stretch">
                        <LabHandshakePanel accent={accent} sectionId={sectionId} phases={labPhases} visualizerRef={visualizerRef} />
                        <div className="p-4 md:p-6 rounded-3xl bg-slate-950/70 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)] flex flex-col gap-4">
                            <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono">
                                <div className="flex items-center gap-2">
                                    <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: accent }} />
                                    <span>Dual visualizer handshake</span>
                                </div>
                                <span className="text-slate-500">phase captions</span>
                            </div>
                            <p className="text-sm text-slate-200 leading-relaxed">
                                Legend chips now call out palette labels while the morph/crystal swap syncs mask intensity, blur, and hue so the invert stays crisp through the full scroll journey.
                            </p>
                            <div className="grid grid-cols-2 gap-3 text-sm text-slate-200">
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                    <div className="text-[11px] uppercase tracking-[0.24em] font-mono text-white/70 mb-1">Mask + blur sync</div>
                                    <p className="leading-relaxed">Morph + crystal canvases now share mask intensity curves to avoid opacity stacking artifacts.</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                    <div className="text-[11px] uppercase tracking-[0.24em] font-mono text-white/70 mb-1">Phase captions</div>
                                    <p className="leading-relaxed">Each phase carries a caption and cue so the lab story stays legible in the HUD.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div id="emergency-journey" className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr] items-stretch">
                    <EmergencyJourneyTile data={EMERGENCY_JOURNEY} accent={accent} visualizerRef={visualizerRef} />
                    ...
                    <div className="p-4 md:p-6 rounded-3xl bg-slate-950/70 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)] flex flex-col gap-4">
                        <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono">
                            ...
                            <span className="text-white/50">visual + content sync</span>
                        </div>

                        <p className="text-sm text-slate-200 leading-relaxed">
                            This tile is wired into the VIB34D visualizer so every hover acts like a rehearsal: the grid, rotation, and glitch envelopes are tuned to the session cadence. Use it to broadcast incidents, guide users through recovery, and keep the morphing focus-content readable even when the rest of the site is in motion.
                        </p>

                        <div className="grid gap-3 md:grid-cols-2">
                            <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                <div className="text-[11px] uppercase tracking-[0.25em] font-mono text-white/70 mb-1">Visualizer choreography</div>
                                <p className="text-sm text-slate-200 leading-relaxed">Session hover ramps patternIntensity and rotationSpeed inside safe bounds, while Session 02 briefly lifts glitchIntensity to spotlight live ops ribbons.</p>
                            </div>
                            <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                                <div className="text-[11px] uppercase tracking-[0.25em] font-mono text-white/70 mb-1">Content journey</div>
                                <p className="text-sm text-slate-200 leading-relaxed">Copy blocks map to the three-session plan: beacon + guard, choreographed updates, then debrief and comments with a cooled visual backdrop.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr] items-stretch">
                    <div className="p-4 md:p-6 rounded-3xl bg-slate-950/65 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)]">
                        <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono mb-4">
                            <div className="flex items-center gap-2">
                                <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: accent }} />
                            ...
                            <span className="text-slate-500">turn-5 // shine</span>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {polishPasses.map((pass, index) => (
                                <div
                                    key={pass.title}
                                    className="p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-white/30 transition shadow-inner"
                                    onMouseEnter={() => {
                                        visualizerRef.current?.updateParams({
                                            ...baseProfile,
                                            patternIntensity: (baseProfile.patternIntensity ?? 1) + 0.14 + index * 0.04,
                                            rotationSpeed: (baseProfile.rotationSpeed ?? 0.2) + 0.05,
                                            glitchIntensity: Math.max(0, (baseProfile.glitchIntensity ?? 0.02) - 0.01 * index),
                                        });
                                    }}
                                    onMouseLeave={() => visualizerRef.current?.updateParams(baseProfile)}
                                >
                                    <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.24em] text-slate-300 font-mono">
                                        ...
                                        <span className="px-2 py-0.5 rounded-full border border-white/15 bg-white/10 text-white/80">{pass.effect}</span>
                                    </div>
                                    <p className="mt-3 text-sm text-slate-200 leading-relaxed">{pass.detail}</p>
                                    <div className="mt-4 h-1 rounded-full bg-white/5 overflow-hidden">
                                        <div className="h-full" style={{ width: `${60 + index * 12}%`, background: accent }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="p-4 md:p-6 rounded-3xl bg-slate-950/65 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)]">
                        <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono mb-4">
                            <div className="flex items-center gap-2">
                                <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: accent }} />
                                <span>Phased Testing & Docs</span>
                            </div>
                            <span className="text-slate-500">qa → publish</span>
                        </div>

                        <div className="space-y-3">
                            {validation.map((item) => (
                                <div key={item.test} className="p-4 rounded-2xl bg-white/5 border border-white/10 flex items-start justify-between gap-3">
                                    <div>
                                        <div className="text-[11px] uppercase tracking-[0.24em] font-mono text-white/80">{item.phase}</div>
                                        <p className="text-sm text-slate-200 leading-relaxed">{item.test}</p>
                                        <p className="text-xs text-slate-400 font-mono mt-1">{item.docs}</p>
                                    </div>
                                    <span className={`px-3 py-1 rounded-full border text-[11px] font-mono uppercase tracking-[0.25em] ${item.state === 'ready' ? 'border-emerald-300 text-emerald-200' : 'border-amber-300 text-amber-200'}`}>
                                        {item.state === 'ready' ? 'Ready' : 'Todo'}
                                    </span>
                                </div>
                            ))}
                        </div>
                        ...
                        <p className="mt-4 text-xs text-slate-400 font-mono leading-relaxed">
                            We keep a standing checklist for smoke, load, and documentation so every visual change ships with a reproducible test and a written record.
                        </p>
                    </div>
                </div>
                ...
                {publication && (
                    <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr] items-stretch">
                        <div className="p-4 md:p-6 rounded-3xl bg-slate-950/70 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)]" onMouseEnter={() => visualizerRef.current?.updateParams({ ...baseProfile, patternIntensity: (baseProfile.patternIntensity ?? 1) + 0.1 })} onMouseLeave={() => visualizerRef.current?.updateParams(baseProfile)}>
                            <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono mb-4">
                                <div className="flex items-center gap-2">
                                    <span className="h-2 w-2 rounded-full animate-ping" style={{ background: accent }} />
                                    <span>Publication Pulse</span>
                                </div>
                                <span className="text-slate-500">seo + governance</span>
                            </div>

                            <div className="space-y-3">
                                {publication.checklist.map((item) => (
                                    <div key={item.title} className="p-4 rounded-2xl bg-white/5 border border-white/10 flex items-start justify-between gap-3">
                                        <div>
                                            <div className="text-[11px] uppercase tracking-[0.24em] font-mono text-white/80">{item.title}</div>
                        ...
                        <div className="p-4 md:p-6 rounded-3xl bg-slate-950/70 border border-white/10 backdrop-blur-xl shadow-[0_16px_70px_rgba(0,0,0,0.35)]">
                            <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-slate-400 font-mono mb-4">
                                <div className="flex items-center gap-2">
                                    <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: accent }} />
                                    <span>Distribution Stack</span>
                                </div>
                                <span className="text-slate-500">feeds + social</span>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-200">
                                {publication.metrics.map((metric) => (
                                    <div key={metric.label} className="p-4 rounded-2xl bg-white/5 border border-white/10 shadow-inner">
                                        <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.24em] font-mono text-slate-300">
                                            <span>{metric.label}</span>
                                            <span className="px-2 py-0.5 rounded-full border border-white/20 bg-white/10 text-white/80">{metric.value}</span>
                                        </div>
                                        <p className="mt-2 text-sm text-slate-200 leading-relaxed">{metric.hint}</p>
                                        <div className="mt-3 h-1.5 rounded-full bg-white/5 overflow-hidden">
                                            <div className="h-full" style={{ width: metric.value.includes('%') ? metric.value : '80%', background: accent }} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <p className="mt-4 text-xs text-slate-400 font-mono leading-relaxed">
                                Publication signals highlight SEO coverage, consent-aware telemetry, and distribution readiness so the blog operates like a professional publication platform.
                            </p>
                        </div>
                    </div>
                )}
