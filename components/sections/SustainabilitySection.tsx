...
const SustainabilitySection: React.FC<SectionProps> = ({ visualizerRef }) => {
    ...
    const handleHover = () => {
        visualizerRef.current?.updateParams({
            morphFactor: 0.9,
            rotationSpeed: 0.1, // Slow down time
            shellWidth: 0.04
        });
    };
            ...
            <div className="min-h-[120vh] w-full flex flex-col items-center pt-32 pb-32 px-4">
                ...
                <div ref={listRef} className="grid grid-cols-1 gap-6 max-w-4xl w-full">
                    {BLOG_CONTENT.sustainability.map((topic, i) => (
                        <div 
                            key={i} 
                            className="wave-card relative bg-slate-900/50 border border-white/5 rounded-xl p-8 flex flex-col md:flex-row md:items-center gap-6 overflow-hidden group hover:border-cyan-500/30 transition-colors duration-500 cursor-pointer"
