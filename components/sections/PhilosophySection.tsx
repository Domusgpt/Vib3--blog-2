    ...
    return (
        <div ref={containerRef} className="w-full h-full overflow-y-auto custom-scrollbar relative">
            <div className="min-h-[120vh] w-full flex flex-col items-center pt-32 pb-32 px-4">
                ...
                <div 
                    ref={gridRef} 
                    className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-purple-500/20 border border-purple-500/20 w-full max-w-6xl"
                    onMouseEnter={handleGridHover}
                    onMouseLeave={handleGridLeave}
                >
                    {BLOG_CONTENT.philosophy.map((item, i) => (
                        <div key={i} className="bg-slate-950/90 p-10 aspect-square flex flex-col items-start justify-between text-left group grid-cell-hover transition-all duration-500 relative overflow-hidden cursor-pointer hover:bg-purple-900/20">
